...
void Conv2DTransposeLayerGl::getWeightConstants(std::vector<WeightContants>& weightConstants, const std::vector<std::vector<float>>& vWeightMatrices,
...
void Conv2DTransposeLayerGl::getBatchNormalizationConstants(std::vector<std::ostringstream>& batchNormalizationConstants, const int shaderPass,
...
void Conv2DTransposeLayerGl::buildPreDefine(std::ostringstream& stream, const LayerGenOptions& options, const std::string& shaderFileName) const {
...
void Conv2DTransposeLayerGl::buildRgbaDefine(std::ostringstream& stream, uint32_t outputChannels) const {
...
void Conv2DTransposeLayerGl::buildFragmentPostDefine(std::ostream& stream, const LayerGenOptions& options) const {
    // if (shader.isRGBA) {
...
void Conv2DTransposeLayerGl::buildComputePostDefine(std::ostream& stream, const LayerGenOptions& options, uint32_t outputSlice) const {
    stream << (!_desc.activation.compare("relu")
                ? "s = max(s, vec4(0.0));\n"
                : !_desc.activation.compare("tanh")
                        ? "s = tanh(s);\n"
...
InferencePassesSptr Conv2DTransposeLayerGl::createFS(const LayerGenOptions& options) const {
    ...
    for (uint32_t shaderPassIndex = 0; shaderPassIndex < numShaderPasses; ++shaderPassIndex) {
        ...
        InferencePassGl& pass = passes[shaderPassIndex];
        ...
        createShaderLayerCode(pass, shaderCode, weightConstants, batchNormalizationConstants, shaderPassIndex, outputChannels);
        ...
        pass.program = InferencePassGl::FsProgram {shaderPassIndex, DIV_4_ROUND_UP(outputChannels)};
    }
...
InferenceGraph::Transform Conv2DTransposeLayerGl::getOutputScaleDimAdjustment() const {
