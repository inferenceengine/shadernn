...
static constexpr const char* ADD_FS_ASSET_NAME = "shaders/shadertemplate_fs_add_RGBA.glsl";
static constexpr const char* ADD_CS_ASSET_NAME = "shaders/3rdparty/shadertemplate_cs_add.glsl";

ShaderLayer::GLSLShaders AddLayer::createFS(const GenericModelLayer::LayerGenOptions& options) const {
    (void) options;
    GLSLShaders ret;
                                        ...
                                        : !_desc.activation.compare("sigmoid")
                                                ...
                                                : !_desc.activation.compare("leakyRelu")
                                                    ...
                                                    : !_desc.activation.compare("leaky_relu")
                                                            ? "\t\ts" + layer2Str + " = max(s" + layer2Str + ", (s" + layer2Str + " * vec4(" +
                                                                std::to_string(_desc.leakyReluAlpha) + "f)));\n"
                                                            : !_desc.activation.compare("SiLU") ? "s" + layer2Str + " = s" + layer2Str +
                                                                                                    " * vec4(1.0f)/(vec4(1.0f)+ exp(-s" + layer2Str + "))"
                                                                                                : "");
...
ShaderLayer::GLSLShaders AddLayer::createCS(const LayerGenOptions& options) const {
    (void) options;
    ...
    string shaderMain = "layout(location=3) uniform ivec4 imgSize;\n"
    ...
    shaderMain = loadShader(ADD_CS_ASSET_NAME);
    ...
    pass.uniforms = {{"imgSize", glm::ivec4(inputWidth, inputHeight, ic_4, 1)}};
