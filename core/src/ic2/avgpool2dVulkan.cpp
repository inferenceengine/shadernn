...
InferencePassesSptr AveragePooling2DLayerVulkan::createCS(const LayerGenOptions& options) const {
    ...
    getPaddingOffsetOrig(paddingOffsets, _desc.padding, _desc.padding, _desc.padding, _desc.padding, _desc.kernelSize);
    ...
    if (_desc.preferHp) {
        ...
        bytes = snn::loadEmbeddedAsset(AVGPOOL2D_VK_ASSET_NAME);
        pass.source = AVGPOOL2D_VK_ASSET_NAME;
    }
