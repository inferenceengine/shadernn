...
absl::StatusOr<std::unique_ptr<Driver>> Driver::CreateExternally(
      DynamicSymbols *symbols, VkInstance instance) {
  UVKC_RETURN_IF_ERROR(symbols->LoadFromInstance(instance));

  return absl::WrapUnique(new Driver(instance, *symbols, true));
}
...
Driver::Driver(VkInstance instance, const DynamicSymbols &symbols, bool externallyOwned)
    : instance_(instance), symbols_(symbols), externallyOwned_(externallyOwned) {}
