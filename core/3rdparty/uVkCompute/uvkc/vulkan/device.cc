...
absl::StatusOr<std::unique_ptr<Device>> Device::CreateExternally(
      VkPhysicalDevice physical_device, VkPhysicalDeviceMemoryProperties memory_properties, VkQueue queue, uint32_t queue_family_index,
      uint32_t valid_timestamp_bits, uint32_t nanoseconds_per_timestamp_value,
      VkDevice device, VkCommandPool command_pool, const DynamicSymbols &symbols) {
  return absl::WrapUnique(new Device(
      device, physical_device, memory_properties, queue, queue_family_index, valid_timestamp_bits,
      nanoseconds_per_timestamp_value, command_pool, symbols));
}
...
absl::Status Device::AttachImageToDescriptor(
  ...
  for (int i = 0; i < bound_images.size(); ++i) {
    ...
    UVKC_LOGV("Descriptor[%d]: set: %d, binding: %d, VkImage: %p, VkImageView: %p, type: %s", i, bound_images[i].set, bound_images[i].binding,
      bound_images[i].image->image(), bound_images[i].image->image_view(), GetVkDescriptorTypeName(binding_info->descriptorType));
    ...
    */
    ...
    if (image_layouts) {
      image_layouts->push_back(info.imageLayout);
    }
  }
...
absl::StatusOr<std::unique_ptr<CommandBuffer>> Device::AllocateCommandBuffer() {
  ...
  VK_RETURN_IF_ERROR(symbols_.vkAllocateCommandBuffers(device_, &allocate_info,
                                                       &command_buffer));
  return std::make_unique<CommandBuffer>(device_, command_buffer, command_pool_, symbols_);
}
...
absl::Status Device::QueueSubmitAndWait(const CommandBuffer &command_buffer) {
  ...
  {
  UVKC_PROFILE_TIME(vkWaitForFences, "Vulkan wait for fence")
...
Device::Device(VkDevice device, VkPhysicalDevice physical_device, VkPhysicalDeviceMemoryProperties memory_properties,
               VkQueue queue, uint32_t queue_family_index, uint32_t valid_timestamp_bits,
               uint32_t nanoseconds_per_timestamp_value,
               VkCommandPool command_pool, const DynamicSymbols &symbols)
    : device_(device),
      physical_device_(physical_device),
...
PFUN_TIMER_FACTORY gExternalTimerFactory = nullptr;

void SetExternalTimerFactory(PFUN_TIMER_FACTORY factory) {
    gExternalTimerFactory = factory;
}
