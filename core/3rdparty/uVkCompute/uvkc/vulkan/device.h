...
class Device {
 public:
  ...
  static absl::StatusOr<std::unique_ptr<Device>> CreateExternally(
  ...
  absl::Status AttachImageToDescriptor(
      ...
      std::vector<VkImageLayout>* image_layouts = nullptr);
  ...
  VkPhysicalDevice getDevice() { return physical_device_;}

  VkDevice getLogicalDevice() { return device_; }

  const DynamicSymbols& getSymbols() { return symbols_; }
 ...
 private:
  ...
  Device(VkDevice device, VkPhysicalDevice physical_device, VkPhysicalDeviceMemoryProperties memory_properties,
