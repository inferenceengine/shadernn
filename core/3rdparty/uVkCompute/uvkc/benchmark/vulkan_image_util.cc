...
absl::Status SetDeviceImageViaStagingBuffer(
  ...
  UVKC_RETURN_IF_ERROR(
      cmdbuffer->TransitionImageLayout(*device_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL));
  cmdbuffer->CopyBufferToImage(*staging_buffer, 0, *device_image,
                               image_dimensions);
  UVKC_RETURN_IF_ERROR(cmdbuffer->TransitionImageLayout(*device_image, to_layout));
...
absl::Status GetDeviceImageViaStagingBuffer(
  ...
  */
  if (device_image->image_layout() != VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL
    && device_image->image_layout() != VK_IMAGE_LAYOUT_GENERAL) {
    UVKC_RETURN_IF_ERROR(cmdbuffer->TransitionImageLayout(
          *device_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL));
  }
