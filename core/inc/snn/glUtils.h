...
#define GLCHK(func)                                                                                                                                        \
    ...
    if (glGetError == NULL) {                                                                                                                              \
        ...
        if (GL_NO_ERROR != err) {                                                                                                                          \
            SNN_LOGE("function %s failed. (error=0x%x)", #func, err);                                                                                      \
        }                                                                                                                                                  \
    }                                                                                                                                                      \
...
public:
    ...
    void allocate2D(snn::ColorFormat f, size_t w, size_t h, size_t channels = 1, size_t m = 1);

    void allocate2DArray(snn::ColorFormat f, size_t w, size_t h, size_t l, size_t channels = 1, size_t m = 1);

    void allocate3D(snn::ColorFormat f, size_t w, size_t h, size_t l, size_t channels = 1, size_t m = 1);

    void allocateCube(snn::ColorFormat f, size_t w, size_t channels = 1, size_t m = 1);
    ...
    snn::ManagedRawImage getBaseLevelPixels() const;
...
public:
    ...
    GLint getUniformBinding(const char* name_) const {
        ...
        // SNN_LOGD("%s:%d bind %s loc: %d\n",__FUNCTION__, __LINE__, name_, loc);
        if (-1 == loc) {
            return -1;
        }
        GLint binding;
...
public:
    ...
    void apply() const {
        ...
        std::visit(
            [&](auto&& v) {
                // SNN_LOGD("%s:%d uniform apply  %s: %d\n",__FUNCTION__, __LINE__, _name.c_str(), _location);
