...
#define TEST_IMAGE        "%s/assets/images/coco1_416.png"
...
int main(int argc, char** argv) {
    ...
    //     formatString("%s/Yolov3-tiny/%s layer [01] Conv2D_7x7 pass[15]_input.dump", DUMP_DIR).c_str(),
    //     3, 64, 32, 7, 0, 2, true);
    ...
    // ASSETS_DIR).c_str(), "flatten_blob", 416); snnMat = getSNNLayerText(formatString("%s/Yolov3-tiny/%s layer [31] Flatten cpu layer.txt", DUMP_DIR,
    ...
    auto ncnnMat13 = getNCNNLayer(formatString("%s/../../modelzoo/Yolov3-tiny/%s", ASSETS_DIR, NCNN_MODEL_NAME).c_str(),
                                  formatString(TEST_IMAGE, ASSETS_DIR).c_str(), "conv2d_9_blob", 416);
    auto ncnnMat26 = getNCNNLayer(formatString("%s/../../modelzoo/Yolov3-tiny/%s", ASSETS_DIR, NCNN_MODEL_NAME).c_str(),
                                  formatString(TEST_IMAGE, ASSETS_DIR).c_str(), "conv2d_12_blob", 416);
    auto snnMat13 =
        getSNNLayer(formatString("%s/Yolov3-tiny/%s layer [20] Conv2D pass[%d].dump", DUMP_DIR, SNN_MODEL_NAME, getPassIndex(63, mrtMode)).c_str(), false, 255);
    auto snnMat26 =
        getSNNLayer(formatString("%s/Yolov3-tiny/%s layer [21] Conv2D pass[%d].dump", DUMP_DIR, SNN_MODEL_NAME, getPassIndex(63, mrtMode)).c_str(), false, 255);
