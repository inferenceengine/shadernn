                                                              ...
                                                              void *                                      /*user_data*/)
{
    ...
    if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) {
        SNN_LOGW("%d - %s: %s", callback_data->messageIdNumber, callback_data->pMessageIdName, callback_data->pMessage);
    }
    else if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) {
        SNN_LOGE("%d - %s: %s", callback_data->messageIdNumber, callback_data->pMessageIdName, callback_data->pMessage);
    }
    return VK_FALSE;
}
                                                     ...
                                                     const char *layer_prefix, const char *message, void * /*user_data*/)
{
    if (flags & VK_DEBUG_REPORT_ERROR_BIT_EXT) {
        SNN_LOGE("%s: %s", layer_prefix, message);
    }
    else if (flags & VK_DEBUG_REPORT_WARNING_BIT_EXT) {
        SNN_LOGW("%s: %s", layer_prefix, message);
    }
    else if (flags & VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT) {
        SNN_LOGW("%s: %s", layer_prefix, message);
    }
    else {
        SNN_LOGI("%s: %s", layer_prefix, message);
    }
    return VK_FALSE;
}
#endif
                     ...
                     const std::vector<VkLayerProperties> &available)
{
    for (auto layer : required) {
        ...
        if (!found) {
            SNN_LOGE("Validation Layer %s not found", layer);
            return false;
        }
    }
...
std::vector<const char *> get_optimal_validation_layers(const std::vector<VkLayerProperties> &supported_instance_layers)
{
    ...
    for (auto &validation_layers : validation_layer_priority_list) {
        ...
        SNN_LOGW("Couldn't enable validation layers (see log for error) - falling back");
    }
                      ...
                      std::vector<const char *> &               enabled_extensions)
{
    for (auto &avail_ext_it : available_exts) {
        if (strcmp(avail_ext_it.extensionName, required_ext_name) == 0) {
            ...
            if (it != enabled_extensions.end()) {
                ...
                SNN_LOGI("Extension %s found, enabling it", required_ext_name);
    ...
    SNN_LOGE("Extension %s not found", required_ext_name);
    return false;
}
                   ...
                   uint32_t                                      api_version)
{
    ...
    if (!has_debug_utils) {
        has_debug_report = enable_extension(VK_EXT_DEBUG_REPORT_EXTENSION_NAME,
                                            available_instance_extensions, enabled_extensions);
        if (!has_debug_report) {
            SNN_LOGW("Neither of %s or %s are available; disabling debug reporting",
    ...
    bool validation_features = false;
    {
        ...
        for (auto &available_extension : available_layer_instance_extensions) {
            if (strcmp(available_extension.extensionName, VK_EXT_VALIDATION_FEATURES_EXTENSION_NAME) == 0) {
                ...
                SNN_LOGI("% is available, enabling it", VK_EXT_VALIDATION_FEATURES_EXTENSION_NAME);
    ...
    if (headless) {
        ...
        if (!has_headless_surface) {
            SNN_LOGW("%s is not available, disabling swapchain creation", VK_EXT_HEADLESS_SURFACE_EXTENSION_NAME);
        }
    }
    else {
    ...
    for (auto extension : required_extensions) {
        ...
        if (!enable_extension(extension_name, available_instance_extensions, enabled_extensions)) {
            if (extension_is_optional) {
                SNN_LOGW("Optional instance extension %s not available, some features may be disabled", extension_name);
            }
            else {
                SNN_LOGE("Required instance extension %s not available, cannot run", extension_name);
...
PhysicalDevice &Instance::get_first_gpu()
{
    ...
    SNN_LOGW("Couldn't find a discrete physical device, picking default GPU");
    return *gpus.at(0);
}
